# Matrix-K-Means function Version 2 for clustering matrix variate data
# Must be a square matrix
# n x n x N Matrix
# allows initialization for clusters.

using StatsBase, Distances, Statistics


function mkmeansv2(data; k = 1 , max_iter = 100, threshold = 0.001, metric = sqeuclidean, init = 1)
        ...
        elseif init != 1
            if(size(init,3) != k)
                error("Size of initialization centroids do not match k\n")
            end
            centroids = init;
        end
